      ...
      <div class="row">
        <nav class="col-md-3">
          <ul class="sideList pl-0" @click.prevent="getFilter">
            <li>
              <a href="#" class="nav-link" :class="{ active: filter === 'All' }"
                >All</a
              >
            </li>
            <li>
        ...
        <section class="col-md-9 products-section">
          <ul
            class="d-flex flex-wrap products-list"
            v-if="filterProducts.length !== 0"
          >
            <li
              v-for="item in filterProducts.slice(count, count + 9)"
              :key="item.id"
            >
              <router-link
                :to="{
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    >
                      ...
                      <a
                        href="#"
                        class="far fa-heart nav-link pr-0"
                        v-if="!favItemsId.includes(item.id)"
                        @click.prevent="addFav(item)"
                      ></a>
                      <!-- 實心愛心 -->
                      <a
                        href="#"
                        class="fas fa-heart nav-link pr-0"
                        v-else
          ...
          <div
            class="d-flex justify-content-center align-items-center wrong-alert"
            v-else
          >
            <h5>很抱歉! 找不到您想要的商品</h5>
          </div>
...
export default {
  ...
  methods: {
    ...
    getFilterFromHome (text) {
      // 自首頁點擊賣場連結時依傳送的filter值篩選
      if (!text) {
        return;
      }
      const vm = this;
    ...
    buildPageForm (len) {
      // 接收computed陣列長度，重新組合頁數資訊傳送給pagination
      const vm = this;
