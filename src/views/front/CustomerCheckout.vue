        ...
        <div class="container order text-primary">
            <div class="row">
                <div class="col-md-7 col p-4 m-auto">
                    ...
                    <button class="btn rounded-0 btn-sm text-primary showdata" type="button" @click="open=!open">
                        + 展開訂單明細
                    </button>
                                ...
                                <tr v-for="(item,key) in order.products" :key="key">
                                    <td class="d-flex">
                                        ...
                                        <div class="d-inline-block pl-2 text-primary">
                                            <router-link :to="{name:'productDetail',params:{productId:item.product_id}}">{{item.product.title}}</router-link><br>
                                            {{item.qty}} {{item.product.unit}}
                                        </div>
                                    </td>
                                    <td class="text-right d-none d-md-table-cell" :class="{'delete':item.total!==item.final_total}" >{{item.total | currency}}元</td>
                                    <td class="text-right">{{item.final_total | currency}}元</td>
                                </tr>
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="4" class="text-right bg-primary text-white h5">
                    ...
                    <div class="mt-4 text-right">
                        <button type="button" class="btn btn-outline-primary" @click.prevent="completeOrder">確認付款</button>
                    </div>
                </div>
            </div>
        </div>
