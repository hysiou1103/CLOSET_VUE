                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="item in carts.carts" :key="item.id">
                                    ...
                                    <td>
                                        <router-link to="{pa}">{{item.product.title}}</router-link><br>
                                        <span> {{item.qty}} {{item.product.unit}} </span>
                                        <span v-if="item.total!==item.final_total" class="hasCoupon"> {{item.total | currency}}元</span>
                                        <span> {{item.final_total | currency}}元</span>
                                    </td>
                                    <td class="unit">
                                        {{item.qty}} {{item.product.unit}}
                                    </td>
                                    <td class="text-right hasCoupon" v-if="item.total!==item.final_total">{{item.total | currency}}元</td>
                                    <td class="text-right final_total">{{item.final_total | currency}}元</td>
                                </tr>
                            </tbody>
                                    ...
                                    <td colspan="3">
                                        <div class="useCoupon input-group py-2">
                                            <input class="form-control mr-3" type="text"
                                                placeholder="請輸入優惠券代碼" v-model="code.code">
                                            <div class="input-group-append">
                                                <button class="btn btn-outline-warning rounded"
                                                    @click="useCoupon">Use it!
                                                </button>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td colspan="3">
                                        <div class="input-group input-group-sm py-2">
                                            <input class="form-control mr-2" type="text"
                                                placeholder="請輸入優惠券" v-model="code.code">
                                            <div class="input-group-append">
                                                <button class="btn btn-outline-warning rounded "
                                                    @click="useCoupon">Use it!
                                                </button>
                            ...
                            <form @submit.prevent="creatOrder">
                                    ...
                                    <div class="form-group col-6">
                                        <ValidationProvider rules="required" v-slot="{errors,classes}">
                                            ...
                                            <input type="text" class="form-control" name="姓名"
                                                id="name"  v-model="form.user.name"   :class="classes">
                                    ...
                                    <div class="form-group col-6">
                                        <ValidationProvider rules="required" v-slot="{errors,classes}">
                                            ...
                                            <input type="tel" class="form-control"  name="電話"
                                                id="tel" v-model="form.user.tel" :class="classes">
                                            <span class="invalid-feedback">{{errors[0]}}</span>
                                        </ValidationProvider>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <ValidationProvider rules="required|email" v-slot="{errors,classes}">
                                        ...
                                        <input type="email" class="form-control" name="信箱"
                                            id="email" v-model="form.user.email" :class="classes">
                                        <span class="invalid-feedback">{{ errors[0] }}</span>
                                    </ValidationProvider>
                                </div>
                                <div class="form-group">
                                    <ValidationProvider rules="required" v-slot="{errors,classes}">
                                        ...
                                        id="address" v-model="form.user.address" :class="classes">
                                        <span class="invalid-feedback">{{ errors[0] }}</span>
                                    </ValidationProvider>
                                </div>
                                <div class="form-group">
                                ...
                                <small class="text-primary">※下單前請再次確認您的購買明細及配送資訊，後續無法修正訂單內容</small><br>
                                <small><a href="#couponInput">※別忘了使用您的優惠券囉!</a></small>
                                <div class="text-center mt-2">
                                    <button type="submit" class="btn btn-outline-primary"  :disabled="invalid" :class="{'prohibit':invalid}">確認送出訂單</button>
                                </div>
                            </form>
                        </validation-observer>
                    </div>
...
export default {
  ...
  methods: {
    ...
    useCoupon () { // 使用優惠券，觸發後重新取得購物車
      const vm = this
      vm.$store.dispatch('isLoading', true)
      const api = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/coupon`
      this.$http.post(api, { data: vm.code }).then(response => {
        if (response.data.success) {
