            ...
            <div class="row">
                ...
                <div class="col-md-8 col pt-md-5">
                    <swiper class="swiper" :options="swiperOption">
                        <swiper-slide class="pupolarItems" v-for="item in products" :key="item.id">
                            <div class="card">
                                <div class="img">
...
export default {
  ...
  methods: {
    getPopularProduct () { // 只篩選出有特價的商品
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/products/all`
      this.$http.get(api).then((response) => {
        if (response.data.success) {
          vm.products = response.data.products.filter(item => item.origin_price && item.is_enabled)
        }
      })
        .catch(err => { vm.$bus.$emit('messagePush', err, 'warning') })
    }
  },
  created () {
    const vm = this
